...
import { Briefcase, Building2, Eye, FileText, Trash } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Plus } from "lucide-react";
import { Company as CompanyType } from "@/type";
import { Globe, Trash2, Image } from "lucide-react";
...
const Company = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-blue-500 p-6 border-b">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              <div
                className="h-10 w-10 rounded-lg bg-blue-50 dark:bg-blue-900 flex items-center
              justify-center"
              >
            ...
            <CardDescription className="text-sm mt-1 text-white">
              Manage your registered companies ({companies.length}/10)
            </CardDescription>
          ...
          {companies.length > 0 ? (
            <div className="grid gap-4">
              {companies.map((c) => (
                ...
                border-2 hover:border-blue-500 transition-all bg-background"
                >
                  <div
                    className="h-16 w-16 rounded-full mx-2 border-2 overflow-hidden
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-lg mt-2 mx-1 truncate">
                      {c.name}
                    </h3>
                    <p className="text-sm mx-1 opacity-70 line-clamp-2 mb-2">
                      {c.description}
                    </p>
                    <a
                      href={c.website}
                      target="_blank"
                      className="text-xs
                    text-blue-500 hover:underline flex mx-1 items-center gap-1 mb-4"
                    >
                      <Globe size={12} />
                      {c.website}
                    </a>
            ...
            <>
              <div className="text-center py-12">
                ...
                <CardDescription className="text-base py-4">
                  No companies registedred yet
                </CardDescription>
                <p className="text-sm opacity-60">
                  Add your company to start posting your job{" "}
                </p>
              </div>
            </>
          )}
      ...
      <Dialog>
        {/* <DialogTrigger> */}
        <DialogTrigger className="hidden" ref={addRef}></DialogTrigger>
        {/* </DialogTrigger> */}
        <DialogContent className="sm:max-w-137.5">
          ...
          <div className="space-y-5 py-4">
            <div className="space-y-2">
              <Label
                htmlFor="name"
                className="text-sm dont-medium flex items-center gap-2"
              >
                <Briefcase size={16} />
                Company Name
              </Label>
