      ...
      <CardHeader className="space-y-4 pb-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <Link href={`/company/${job.company_id}`} className="shrink-0">
            <div
              className="w-14 h-14 rounded-xl border-2 overflow-hidden hover:scale-105 transition-transform
            bg-background"
            >
              <img
      ...
      <CardContent className="flex flex-col gap-3 pt-4 border-t">
        <div className="flex w-full gap-2">
          ...
          {user && user.role === "jobseeker" && (
            <>
              {applied ? (
                <div
                  className="flex-1 flex items-center justify-center gap-2 text-green-600
        ...
        {job.is_active === false && (
          <div
            ...
            text-red-600 bg-red-100 dark:bg-red-900/30 rounded-md px-3 py-2 font-medium"
          >
            Position Closed
          </div>
        )}
