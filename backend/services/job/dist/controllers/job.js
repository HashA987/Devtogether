...
export const createCompany = TryCatch(async (req, res) => {
    ...
    if (existingCompanies.length > 0) {
        throw new ErrorHandler(409, `company with the same name ${name} already exist`);
    }
    const file = req.file;
    if (!file) {
...
export const deleteCompany = TryCatch(async (req, res) => {
    ...
    if (!company) {
        throw new ErrorHandler(404, "company not found,you are not allowed to delete");
    }
...
export const createJob = TryCatch(async (req, res) => {
    ...
    const [company] = await sql `SELECT company_id FROM companies WHERE company_id =
...
export const upadateJob = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
        throw new ErrorHandler(401, "Authentication required");
    }
    ...
    const [existingJob] = await sql `SELECT posted_by_recruiter_id FROM jobs WHERE job_id =
  ${req.params.jobId}`;
    if (!existingJob) {
...
export const getAllApplicationForJob = TryCatch(async (req, res) => {
    ...
    const applications = await sql `SELECT * FROM applications WHERE job_id = ${jobId} ORDER BY applied_at ASC`;
...
export const updateApplication = TryCatch(async (req, res) => {
    ...
    const { id } = req.params;
    const [application] = await sql `SELECT * FROM applications WHERE application_id
    = ${id}`;
    if (!application) {
